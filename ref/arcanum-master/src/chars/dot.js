...
export default class Dot {
	toJSON() {
		...
		return {
			...
			checkTags: this.checkTags?.join(",") || undefined,
	...
	set checkTags(v) {
		if (!v) return;
		if (typeof v === "string") this._checkTags = v.split(",").map(t => t.trim());
		else if (Array.isArray(v)) this._checkTags = v;
		else console.warn("Unknown checkTags in setter", v);
	}
