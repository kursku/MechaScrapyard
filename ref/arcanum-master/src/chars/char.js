...
export default class Char {
	...
	addDot(dot, source, duration = 0, applier = null) {
		...
		let tags = Array.isArray(dot.tags) ? dot.tags : dot.tags ? dot.tags?.split(",") : [];
		let checkTags;
		...
		if (dot.checkTags != null) {
			checkTags = Array.isArray(dot.checkTags) ? dot.checkTags : dot.checkTags ? dot.checkTags?.split(",") : [];

			if (dot._checkTags) {
				if (Array.isArray(dot._checkTags) && dot.checkTags !== dot._checkTags) {
					dot._checkTags.forEach(t => {
						if (!checkTags.includes(t)) checkTags.push(t);
					});
				}
				delete dot._checkTags;
			}
			dot.checkTags = checkTags;
		} else {
			checkTags = tags;
		}
		...
		let clashingDots = [];
		for (let appliedDot of this.dots) {
			if (appliedDot.id === id) {
				matchingDot = appliedDot;
				break;
			}
			//we check if any of our checkTags match any of the opposing DoT's tags, in which case, we are in opposition.
			if (checkTags.find(t => appliedDot.tags.includes(t))) {
				clashingDots.push(appliedDot);
				continue;
			}
			//we check if any of the OTHER dots checkTags match OUR tags - if they do, we would also be clashing.
			let opposingCheckTags = appliedDot.checkTags ?? appliedDot.tags;
			if (opposingCheckTags.find(t => tags.includes(t))) {
				clashingDots.push(appliedDot);
				continue;
			}
		}

		if (matchingDot && clashingDots.length) {
			console.warn("matching dot found alongside clashing dots", matchingDot, clashingDots);
		}

		if (matchingDot) {
			matchingDot.extend(duration);
			return;
		} else if (clashingDots.length) {
			if (clashingDots.find(d => d.level > level)) {
				// If an existing dot with a clashing tag is a higher level than the dot currently being applied
				return;
			}
			clashingDots.forEach(d => this.removeDot(d));
		}
		/*
		let existingDot = this.dots.find(d => d.id === id || d.tags.find(t => tags.includes(t)));

		if (existingDot != null) {
			if (existingDot.id === id) {
				existingDot.extend(duration);
				return;
			}

			if (existingDot.level <= level) {
