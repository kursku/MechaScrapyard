...
export default class Slot {
	...
	*[Symbol.iterator]() {
	...
	freeSpace(forcedsize) {
		...
		if (count < 0) return this.item.length > 0 ? -1 : 0;
	...
	find(id, proto = false) {
		...
		if (proto) {
			return this.item.find(v => v.id === id || v.recipe === id);
		} else {
			return this.item.find(v => v.id === id);
		}
	}
	...
	slotSizeChange(newSize) {
		...
		while (this.freeSpace() < 0 && (this.savedMax <= newSize || newSize < 0)) {
			if (!this.item) break;
			if (Array.isArray(this.item)) {
