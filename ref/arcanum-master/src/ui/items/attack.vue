...
import { DefaultToMaxCombatants, MaxCombatants } from "@/values/combatVars";
...
export default {
	...
	computed: {
		...
		maxTargets() {
			let maxTargets = !this.attack.maxTargets
				? this.attack.targets & DefaultToMaxCombatants
					? MaxCombatants * 2
					: 1
				: this.attack.maxTargets.valueOf();

			if (maxTargets == 1) return "";
			if (maxTargets == 999 || maxTargets == MaxCombatants * 2) return " (♾️)";
			return " (" + maxTargets + ")";
		},
		potency() {
			let potencystring = "";
		...
		only() {
			...
			for (let o of this.attack.only) {
				...
				onlystring = onlystring.concat(game.getData(o)?.name.toTitleCase() || o.toTitleCase());
			}
