 ...
 * minOp    - Atmod-like greater than or gte check for min, which sets count to 0 if min isn't met (ignoring start). If left blank, sets count to start if count is lower than min.
 ...
 * mode     - Toggles a specific mode for handling steps.
 * 		      "~" - Enables sectioning, meaning the difference between min and max is divided into (step + 1) of sections.
 * 	          "=" - Enables full maximum, meaning count is forcibly set to the maximum value at or past max.
 *                  Matters when steps doesn't nicely divide into the difference between min and max ((max - min) / steps).
 *            By default, goes from min (or 0 if min isnt defined) up to max by step amount.
 *            Max and step must be defined for modes to work properly.
 ...
 *
 * Full Template
 * (modFlat)(modPct)/(minOp)(min)/(max)/(mode)(rounding)(step)
 */
const RangedRegex =
...
export default class RangedMod extends Mod {
	...
	get str() {
		...
		return `${base || ""}${basePct > 0 && base ? "+" : ""}${basePct ? basePct * 100 + "%" : ""}/${this._start != null ? Stat.getBase(this._start) + "/" : ""}${OPS[this.minOp] || ""}${Stat.getBase(this.min) || ""}/${Stat.getBase(this.max) || ""}/${this.mode ? "~" : ""}${this.roundingSym || ""}${Stat.getBase(this.step)}`;
	}
	set str(v) {
		this.parseMod(v);
	}
	...
	estimateStep() {
		...
		return this.mode === ModeOptions.SECTION ? this.range / this.step : +this.step;
	}
	...
	maxed() {
		return this.max != null && this.count >= this.range;
	}
	...
	/**
	 * Adjusts and returns a count based on this RangedMod's parameters (min, max, minOp, start, rounding, step, mode)
	 * @param {Number} count Unmodified count to be adjusted according to this RangedMod
	 * @returns Adjusted mod count
	 */
	adjustCount(count) {
		if (this.max != null && count >= this.max) {
			if (this.mode === ModeOptions.FULL) return this.range + this.start;
			count = this.range;
		} else if (this.min != null && count <= this.min) {
			// Hard set count to 0 if lower than min with operator
			if (count < this.min || (this.minOp === OPS.GT.value && +this.min === count)) return 0;
			count = 0;
		} else {
			count = count - this.min;
		}
		if (this.step) {
			let rounding = roundingFunc(this.roundingSym);
			let { range, step } = this;
			if (this.mode === ModeOptions.SECTION) {
				count = (rounding((count * step) / range) * range) / step;
			} else {
				count = rounding(count / step) * step;
			}
		}
