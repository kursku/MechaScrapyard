...
export const canTarget = (targs, it) => {
	if (Array.isArray(targs)) {
		for (let i = targs.length - 1; i >= 0; i--) {
			const t = targs[i];
			if ((t && t == it.type) || t === it.kind || t === it.slot || t === it.recipe || it.hasTag(t)) return true;
		}
		return false;
	}

	return targs === it.type || targs === it.kind || targs === it.slot || targs === it.recipe || it.hasTag(targs);
};
/**
