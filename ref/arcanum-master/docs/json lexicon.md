...
```JSON
    %item%.cost
```
    describes cost of using/producing items one time.

```JSON
    %item%.run
```
describes cost of keeping action active for 1 second. **Note** that if player can't support this cost, action will stop.
used for continuous actions and skills.

```JSON
    %item%.effect
```
describes rewards of keeping action active for 1 second. **Note** that negative reward can drop target to 0, but will not stop on reaching it.
used for continuous actions and skills.
...
```JSON
    %item%.mod
```
describes reward of getting the item.
...
```JSON
    %item%.require
```
condition that describes unlock requirement.
**note** that item will still be unlocked if conditions will later become false.
**note** that if it's not specified, and `need` is specified, `require` will copy need.

```JSON
    %item%.need
```
condition that describes use requirement.
**note** that item will still be unlocked when not meeting this requirement, but wouldn't be usable.

```JSON
    %item%.every.%n%
```
describes modification to `item` that happens every `n` times player have it.

```JSON
    %item%.at.%n%
```
describes modification to `item` that happens specifically when it used `n` times.
...
```JSON
    %item%.name
```
Name of item displayed in-game.
...
```JSON
    %item%.desc
```
string describing item, that will be displayed in-game.
